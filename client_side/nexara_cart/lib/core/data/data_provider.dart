...
class DataProvider extends ChangeNotifier {
  ...
  Future<List<Product>> getAllProducts({bool showSnack = false}) async {
    // 1. Gọi API lấy danh sách sản phẩm từ server
    try {
      Response response = await service.getItems(endpointUrl: 'products');
      // 2. Nếu server trả về thành công (status code OK)
      if (response.isOk) {
        // 3. Chuyển dữ liệu JSON từ server thành đối tượng ApiResponse<List<Product>>
        ...
        // 4. Lưu danh sách sản phẩm vào biến nội bộ
        _allProducts = apiResponse.data ?? [];
        _filteredProducts = List.from(_allProducts); // copy of _allProducts
        // 5. Thông báo cho UI cập nhật lại (vì dữ liệu đã thay đổi)
        notifyListeners();
        // 6. Nếu có yêu cầu hiển thị thông báo (showSnack = true)
        if (showSnack) {
          SnackBarHelper.showSuccessSnackBar(apiResponse.message);
        }
      }
    } catch (e) {
      // 7. Nếu có lỗi xảy ra trong quá trình gọi API hoặc xử lý dữ liệu
      log(e.toString());
      if (showSnack) {
        SnackBarHelper.showErrorSnackBar(e.toString());
      }
